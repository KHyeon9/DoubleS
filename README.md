...
* Redis 캐싱 전략 도입: 사용자 인증 과정(Security Context Load)에서 발생하는 반복적인 DB Read 부하를 줄이기 위해 Redis Cache를 도입했습니다.
  * 조회 속도를 기존 평균 63 ~ 64ms에서 9ms~10ms 수준으로 약 84% 개선하여 DB 로드를 절감했습니다.
  * 스레드를 활용한 100건의 동시 요청 테스트 했을 때, 평균 1ms 이하 응답 시간 유지를 확인했습니다.
* N+1 문제 해결
...
* 인증 및 권한 검증
  * 비로그인 상태에서 인증이 필요한 API에 접근 시 접근 거부가 발생하는지를 테스트하여 보안 취약점을 사전 방지했습니다.
### 3. 예외 처리 구조
